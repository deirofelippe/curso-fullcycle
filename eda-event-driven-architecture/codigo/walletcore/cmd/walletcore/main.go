...
func main() {
	db, err := sql.Open("mysql", fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8&parseTime=True&loc=Local", "root", "root", "db-walletcore", "3306", "wallet"))
	if err != nil {
		panic(err)
	}
	...
	balanceUpdatedHandler := handler.NewUpdateBalanceKafkaHandler(kafkaProducer)
